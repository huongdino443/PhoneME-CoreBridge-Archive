...
Status Runtime::start_midlet(SuiteId suite_id,
    ...
    std::string image_cache_identity =
        SuiteInstaller::digest_hex(suite.archive_sha256);
    if (!optional_class_archive.empty()) {
        image_cache_identity += "|" + optional_class_archive;
    }
    configured = application_vm->machine.configure_decoded_image_cache(
        runtime_home + "/image-cache/" + std::to_string(suite_id.value) + ".bin",
        std::move(image_cache_identity));
    if (!configured) return fail_start(configured.error());
    // Stable phoneME prepares its suite image cache before entering the game.
    // A valid persistent cache is loaded first; only a cold/stale cache scans
    // and decodes image-like resources. All malformed/unsupported resources
    // remain best-effort and never abort launch.
    (void)application_vm->machine.prewarm_image_cache();
