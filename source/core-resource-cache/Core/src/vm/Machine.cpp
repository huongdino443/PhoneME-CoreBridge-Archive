...
namespace phoneme::vm
{
    namespace
  {
    constexpr char kDecodedImageCacheMagic[] = "CBICIMG1";
    constexpr u32 kDecodedImageCacheVersion = 1U;
    constexpr u32 kDecodedImageCacheMaximumEntries = 4096U;
    constexpr u32 kDecodedImageCacheMaximumNameBytes = 16U * 1024U;

    [[nodiscard]] bool write_cache_u32(std::ostream& output, u32 value)
    {
      const std::array<u8, sizeof(u32)> bytes {
    ...
    [[nodiscard]] bool read_cache_u32(std::istream& input, u32& value)
    {
    ...
    [[nodiscard]] bool write_cache_string(std::ostream& output,
                                          std::string_view value)
    {
      if (value.size() > std::numeric_limits<u32>::max()) return false;
      return write_cache_u32(output, static_cast<u32>(value.size())) &&
          (output.write(value.data(), static_cast<std::streamsize>(value.size())),
           output.good());
    }

    [[nodiscard]] bool read_cache_string(std::istream& input,
                                         std::string& value,
                                         u32 maximum_bytes)
    {
      u32 length = 0U;
      if (!read_cache_u32(input, length) || length > maximum_bytes) return false;
  ...
  void Machine::shutdown() noexcept
  {
    ...
    {
      std::scoped_lock lock(decoded_image_cache_mutex_);
      decoded_image_cache_.clear();
      decoded_image_cache_order_.clear();
      decoded_image_cache_pixel_bytes_ = 0U;
    }
  ...
  std::optional<graphics::Image> Machine::find_cached_decoded_image(
      std::string_view resource_name) const
  {
    std::scoped_lock lock(decoded_image_cache_mutex_);
    const auto iterator = decoded_image_cache_.find(std::string(resource_name));
    if (iterator == decoded_image_cache_.end()) return std::nullopt;
    const graphics::Image &cached = iterator->second;
    // Keep Java ME object ownership unchanged: every createImage call receives
    // an independent immutable pixel buffer, while avoiding ZIP inflate and
    // image format decoding on a cache hit.
  ...
  void Machine::store_cached_decoded_image(std::string resource_name,
                                           graphics::Image image)
  {
    if (resource_name.empty() || image.is_mutable()) return;
    const usize pixel_count = image.pixels().size();
    if (pixel_count == 0U ||
        pixel_count > kDecodedImageCachePixelByteLimit / sizeof(graphics::Pixel))
      return;
    const u64 pixel_bytes = static_cast<u64>(pixel_count) *
                            static_cast<u64>(sizeof(graphics::Pixel));
    std::scoped_lock lock(decoded_image_cache_mutex_);

    auto existing = decoded_image_cache_.find(resource_name);
    if (existing != decoded_image_cache_.end()) {
      decoded_image_cache_pixel_bytes_ -= static_cast<u64>(
          existing->second.pixels().size()) * sizeof(graphics::Pixel);
      decoded_image_cache_.erase(existing);
      decoded_image_cache_order_.erase(
          std::remove(decoded_image_cache_order_.begin(),
                      decoded_image_cache_order_.end(), resource_name),
          decoded_image_cache_order_.end());
    }
    while (decoded_image_cache_pixel_bytes_ + pixel_bytes >
               kDecodedImageCachePixelByteLimit &&
           !decoded_image_cache_order_.empty()) {
      const std::string oldest = decoded_image_cache_order_.front();
      decoded_image_cache_order_.pop_front();
      const auto evicted = decoded_image_cache_.find(oldest);
      if (evicted == decoded_image_cache_.end()) continue;
      decoded_image_cache_pixel_bytes_ -= static_cast<u64>(
          evicted->second.pixels().size()) * sizeof(graphics::Pixel);
      decoded_image_cache_.erase(evicted);
    }
    if (decoded_image_cache_pixel_bytes_ + pixel_bytes >
        kDecodedImageCachePixelByteLimit)
      return;
    decoded_image_cache_pixel_bytes_ += pixel_bytes;
    decoded_image_cache_order_.push_back(resource_name);
    decoded_image_cache_.emplace(std::move(resource_name), std::move(image));
  }

  Status Machine::configure_decoded_image_cache(
      std::string cache_path,
      std::string cache_identity)
  {
    std::scoped_lock lock(decoded_image_cache_mutex_);
    decoded_image_cache_path_ = std::move(cache_path);
    decoded_image_cache_identity_ = std::move(cache_identity);
    decoded_image_cache_prepared_ = false;
    decoded_image_cache_.clear();
    decoded_image_cache_order_.clear();
    decoded_image_cache_pixel_bytes_ = 0U;
    return {};
  ...
  bool Machine::load_decoded_image_cache_file()
  {
    if (decoded_image_cache_path_.empty() ||
        decoded_image_cache_identity_.empty())
      return false;
    std::ifstream input(decoded_image_cache_path_, std::ios::binary);
    if (!input) return false;

    std::array<char, sizeof(kDecodedImageCacheMagic) - 1U> magic {};
    input.read(magic.data(), static_cast<std::streamsize>(magic.size()));
    if (!input.good() ||
        std::memcmp(magic.data(), kDecodedImageCacheMagic, magic.size()) != 0)
      return false;
    u32 version = 0U;
    if (!read_cache_u32(input, version) || version != kDecodedImageCacheVersion)
      return false;
    std::string identity;
    if (!read_cache_string(input, identity, 4096U) ||
        identity != decoded_image_cache_identity_)
      return false;
    u32 entry_count = 0U;
    if (!read_cache_u32(input, entry_count) ||
        entry_count > kDecodedImageCacheMaximumEntries)
      return false;
    ...
    for (u32 index = 0U; index < entry_count; ++index) {
      std::string resource_name;
      u32 width = 0U;
      u32 height = 0U;
      u32 pixel_count = 0U;
      if (!read_cache_string(input, resource_name,
                             kDecodedImageCacheMaximumNameBytes) ||
          resource_name.empty() || !read_cache_u32(input, width) ||
          !read_cache_u32(input, height) || !read_cache_u32(input, pixel_count))
        return false;
      if (width == 0U || height == 0U ||
          width > static_cast<u32>(std::numeric_limits<i32>::max()) ||
          height > static_cast<u32>(std::numeric_limits<i32>::max()))
        return false;
      const u64 expected_pixels = static_cast<u64>(width) * height;
      if (expected_pixels != pixel_count ||
          expected_pixels > kDecodedImageCachePixelByteLimit / sizeof(graphics::Pixel))
        return false;
      std::vector<graphics::Pixel> pixels(pixel_count);
      input.read(reinterpret_cast<char*>(pixels.data()),
                 static_cast<std::streamsize>(pixels.size() * sizeof(graphics::Pixel)));
      if (!input.good()) return false;
      auto image = graphics::Image::create_immutable_owned(
          static_cast<i32>(width), static_cast<i32>(height), std::move(pixels));
      if (!image) return false;
      store_cached_decoded_image(std::move(resource_name), std::move(*image));
    }
    return true;
  }

  void Machine::save_decoded_image_cache_file() const noexcept
  {
    if (decoded_image_cache_path_.empty() ||
        decoded_image_cache_identity_.empty())
      return;
    std::error_code error;
    const std::filesystem::path path(decoded_image_cache_path_);
    ...
    output.write(kDecodedImageCacheMagic,
                 static_cast<std::streamsize>(sizeof(kDecodedImageCacheMagic) - 1U));
    if (!output.good() || !write_cache_u32(output, kDecodedImageCacheVersion) ||
        !write_cache_string(output, decoded_image_cache_identity_)) {
      output.close();
    ...
    std::scoped_lock lock(decoded_image_cache_mutex_);
    const u32 entry_count = static_cast<u32>(std::min<usize>(
        decoded_image_cache_.size(), kDecodedImageCacheMaximumEntries));
    if (!write_cache_u32(output, entry_count)) {
      output.close();
      std::filesystem::remove(temporary, error);
      return;
    }
    u32 written = 0U;
    for (const std::string& resource_name : decoded_image_cache_order_) {
      if (written >= entry_count) break;
      const auto iterator = decoded_image_cache_.find(resource_name);
      if (iterator == decoded_image_cache_.end()) continue;
      const graphics::Image& image = iterator->second;
      const auto pixels = image.pixels();
      if (!write_cache_string(output, resource_name) ||
          !write_cache_u32(output, static_cast<u32>(image.width())) ||
          !write_cache_u32(output, static_cast<u32>(image.height())) ||
          !write_cache_u32(output, static_cast<u32>(pixels.size()))) {
        output.close();
        std::filesystem::remove(temporary, error);
        return;
      }
  ...
  Status Machine::prewarm_image_cache()
  {
    if (decoded_image_cache_prepared_) return {};
    decoded_image_cache_prepared_ = true;
    if (load_decoded_image_cache_file()) return {};

    struct PendingImageDecode final {
      std::string resource_name;
      std::vector<u8> bytes;
      std::optional<graphics::Image> decoded;
    };

    constexpr usize kMaximumBatchEntries = 32U;
    constexpr usize kMaximumBatchBytes = 8U * 1024U * 1024U;
    std::vector<PendingImageDecode> pending;
    usize pending_bytes = 0U;

    const auto decode_pending = [&pending](usize begin, usize end) {
      for (usize index = begin; index < end; ++index) {
        auto decoded = graphics::decode_image(pending[index].bytes);
        if (decoded) pending[index].decoded = std::move(*decoded);
      }
    };

    const auto flush_pending = [&]() {
      if (pending.empty()) return;
      auto& coordinator = runtime::shared_work_coordinator();
      if (coordinator.try_begin_background_work()) {
        runtime::shared_compute_executor().parallel_for(
            runtime::WorkClass::background,
            pending.size(),
            4U,
            1U,
            decode_pending);
        ...
        decode_pending(0U, pending.size());
      }
      for (PendingImageDecode& item : pending) {
        if (item.decoded.has_value()) {
          store_cached_decoded_image(item.resource_name,
                                     std::move(*item.decoded));
        }
      }
      pending.clear();
      pending_bytes = 0U;
    ...
    for (const std::string& resource_name : classes_.image_resource_names()) {
      if (find_cached_decoded_image(resource_name).has_value()) continue;
      auto bytes = classes_.read_resource(resource_name);
      if (!bytes) continue;
      const usize resource_bytes = bytes->size();
      if (!pending.empty() &&
          (pending.size() >= kMaximumBatchEntries ||
           pending_bytes > kMaximumBatchBytes -
               std::min(resource_bytes, kMaximumBatchBytes))) {
        flush_pending();
      }
      pending.push_back(PendingImageDecode {
          .resource_name = resource_name,
          .bytes = std::move(*bytes),
      });
      pending_bytes += resource_bytes;
      if (pending.size() >= kMaximumBatchEntries ||
          pending_bytes >= kMaximumBatchBytes) {
        flush_pending();
      }
    }
    flush_pending();
    save_decoded_image_cache_file();
    return {};
  }
